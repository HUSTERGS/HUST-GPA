	...
	for i in range(int(df.columns.size / 4)):
		...
		for index, credit in enumerate(course_credit):
			...
			if not np.isnan(credit):
				...
				all_info['course_info'].append({'course': course_name[index].strip(), 'grade': course_grade[index], 'credit': course_credit[index],'subGPA': {"HUST": hust_result, "Standard": standard_result, "PKU": pku_result},  'year': str(i + 1), 'is_optional': type(is_optional[index]) == type('')}) 
